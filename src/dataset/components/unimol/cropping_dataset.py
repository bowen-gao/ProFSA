        ...
        if self.max_atoms and len(atoms) > self.max_atoms:
            with data_utils.numpy_seed(self.seed, epoch, index):
                index = np.random.choice(
                    len(atoms), self.max_atoms, replace=False
                )
...
class CroppingResiduePocketDataset(BaseWrapperDataset):
    def __init__(
        self, dataset, seed, atoms, residues, coordinates, max_atoms=256
    ):
        self.dataset = dataset
        self.seed = seed
        self.atoms = atoms
            ...
            with data_utils.numpy_seed(self.seed, epoch, index):
                ...
                for res in residues:
                    if res not in residues_ids:
                        ...
                        residues_distance.append(
                            distance[residues == res].mean()
                        )
                ...
                residues_distance += (
                    1  # prevent inf and smoothing out the distance
                )
                weight = softmax(np.reciprocal(residues_distance))
                max_residues = self.max_atoms // (
                    len(atoms) // (len(residues_ids) + 1)
                )
                if max_residues < 1:
                    max_residues += 1
