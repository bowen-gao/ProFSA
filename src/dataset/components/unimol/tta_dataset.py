...
class TTADataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        coordinates = np.array(
            self.dataset[smi_idx][self.coordinates][coord_idx]
        )
...
class TTADecoderDataset(BaseWrapperDataset):
    def __init__(
        self, dataset, seed, atoms, coordinates, selfies="selfies", conf_size=10
    ):
        self.dataset = dataset
        self.seed = seed
        self.atoms = atoms
...
class TTADockingPoseDataset(BaseWrapperDataset):
    ...
    def __cached_item__(self, index: int, epoch: int):
        ...
        pocket_coordinates = np.array(
            self.dataset[smi_idx][self.pocket_coordinates][0]
        )
        if self.is_train:
            holo_coordinates = np.array(
                self.dataset[smi_idx][self.holo_coordinates][0]
            )
        ...
        return {
            ...
            "holo_pocket_coordinates": holo_pocket_coordinates.astype(
                np.float32
            ),
            "smi": smi,
            "pocket": pocket,
