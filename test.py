...
def default_override(config):
    ...
    if (
        "batch_size" in config.dataset
        and type(config.dataset.batch_size) is int
    ):
...
def test_kahraman(config):
    config_dir = main_dir / "conf"
    with initialize_config_dir(version_base="1.3", config_dir=str(config_dir)):
        cfg = compose(
            config_name="train",
            overrides=["dataset=kahraman", "pipeline=test_kahraman"],
        )
...
def test_toughm1(config):
    config_dir = main_dir / "conf"
    with initialize_config_dir(version_base="1.3", config_dir=str(config_dir)):
        cfg = compose(
            config_name="train",
            overrides=["dataset=toughm1", "pipeline=test_toughm1"],
        )
    config.dataset = cfg.dataset
    config.pipeline = cfg.pipeline
    return config
